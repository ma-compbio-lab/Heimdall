...
class SortingWeightedResampleFc(Fc):
    """Fc that orders tokens by descending expression and keeps the top
    `max_input_length`.  Works with any identity-style Fe. Weighted resample
    context length.
    """
    def __init__(
            self,
    ...
    def tailor(
        ...
        if "medians" in self.adata.var:
            expression_tokenization = expression_tokenization - self.adata.var["medians"].iloc[gene_tokenization].values
        ...
        order = np.argsort(filtered_expression_tokenization)[::-1]
        filtered_gene_tokenization = filtered_gene_tokenization[order]
        filtered_expression_tokenization = filtered_expression_tokenization[order]

        cell_tokenization = np.stack([filtered_gene_tokenization, filtered_expression_tokenization], axis=0)

        if cell_tokenization.shape[1] > self.max_input_length:
            cell_tokenization = self.limit(cell_tokenization)
    ...
    def limit(self, cell_tokenization: NDArray) -> NDArray:
        ...
        order   = np.argsort(expression_values[chosen])[::-1]
        chosen  = chosen[order]
    ...
    def embed_cells(
        ...
        """
        Look up both embeddings and add them element-wise
        """
...
class SortingRandomSampleFc(Fc):
    """
    Fc that orders tokens by descending expression and keeps the top
    `max_input_length`.  Works with any identity-style Fe. Randomly 
    ...
    def limit(self, cell_tokenization: NDArray) -> NDArray:
        ...
        order = np.argsort(expression_values[final_indices])[::-1]
...
class SortingTruncateFc(Fc):
    """
    Fc that orders tokens by descending expression and keeps the top
    `max_input_length`.  Works with any identity-style Fe. Truncates 
    context length.
    """
    def __init__(
            self, 
...
class ChromSortRandomSampleFc(Fc):
    ...
    def embed_cells(
        ...
        expression_embeddings = expression_embedding_layer(expression_inputs.to(torch.float32))
        expression_embeddings[chrom_token_mask] = 0.0
